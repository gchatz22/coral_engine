//! `coral apply` — thin Temporal client with multi-agent YAML support.
//!
...
//! `crates/coral_worker/src/bin/worker.rs`.
//!
...
//!   [`DEFAULT_TASK_QUEUE`] (`coral-agents`).
...
ENV:
    ...
    TEMPORAL_TASK_QUEUE                    Task queue the worker daemon listens on
                                           (default `coral-agents`). Override only
...
async fn run() -> Result<()> {
    ...
    MIGRATOR
        .run(&pool)
        .await
        .context("applying structural-DB migrations (coral_graph::MIGRATOR)")?;
        ...
        Err(GraphStoreError::GraphAlreadyExists { name }) => {
            return Err(anyhow!(
                "graph {name:?} already exists in the structural DB; \
                 `coral apply` is CREATE-only. Drop the existing graph \
    ...
    println!("coral-apply: graph_id={}", applied.graph_id);
    println!(
        "coral-apply: graph_name={} agent_count={}",
        applied.graph_name,
        starts.len()
    );
    for start in &starts {
        ...
        let operator_id = applied
            ...
            .unwrap_or("?");
        println!(
            "coral-apply: started agent {operator_id} workflow_id={}",
            start.workflow_id
        );
    }
    if let Some(first) = starts.first() {
        println!(
            "coral-apply: temporal workflow describe --workflow-id {}",
            first.workflow_id
        );
    }
    Ok(())
...
fn count_agents(graph: &GraphYaml) -> usize {
    fn rec(agent: &coral_graph::yaml::Agent, n: &mut usize) {
        *n += 1;

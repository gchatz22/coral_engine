...
//!
//! **Gated behind BOTH `CORAL_SMOKE_LLM_MCP=1` and `COHERE_API_KEY`.**
...
fn end_to_end_parallel_call_tools_against_server_everything() {
    if std::env::var("CORAL_SMOKE_LLM_MCP").is_err() {
        eprintln!("smoke_llm_mcp_cohere::parallel: skipped (set CORAL_SMOKE_LLM_MCP=1 to run)");
        return;
    }
    if std::env::var("COHERE_API_KEY").is_err() {
        eprintln!("smoke_llm_mcp_cohere::parallel: skipped (COHERE_API_KEY not set)");
        return;
    }
...
fn end_to_end_llm_decide_against_server_everything() {
    if std::env::var("CORAL_SMOKE_LLM_MCP").is_err() {
        eprintln!(
            "smoke_llm_mcp_cohere: skipped (set CORAL_SMOKE_LLM_MCP=1 to run; \

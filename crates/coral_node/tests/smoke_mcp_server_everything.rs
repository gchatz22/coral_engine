...
//!
//! **Gated behind `CORAL_SMOKE_MCP=1`.** Without that env var the test
...
async fn end_to_end_get_sum_against_server_everything() {
    if std::env::var("CORAL_SMOKE_MCP").is_err() {
        eprintln!(
            "smoke_mcp_server_everything: skipped (set CORAL_SMOKE_MCP=1 to run; \

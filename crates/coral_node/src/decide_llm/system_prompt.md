...
3. **Cite your evidence.** `write_output` takes a prose `body` and a `citations` list of evidence ids; every id must resolve in your evidence store or the runtime rejects the output. Evidence comes from tool calls — each `call_tool` result becomes a fresh evidence record a later `write_output` can cite. You keep one Output: each `write_output` replaces it with the current view, it does not append a new one.

//! `node-run` — hand-runnable smoke binary for the `coral_node` crate.
//!
...
//!   (or reused, if it already exists). See [`coral_node::fs::AgentFs`] for
//!   the schema.
//! * `decisions.jsonl` — sibling of `<config.json>` (same directory). One
//!   JSON-serialized [`coral_node::decision::Decision`] per line, in the
...
use coral_node::decision::{Decision, MockDecide};
...
const USAGE: &str = "\
node-run — boot a single coral_node Agent against a config + JSONL fixture.

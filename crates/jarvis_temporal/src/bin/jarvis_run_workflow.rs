...
USAGE:
    jarvis-run-workflow <config.json> <fs_root>
...
fn parse_args(argv: Vec<String>) -> Result<Args> {
    ...
    if argv.len() != 2 {
        return Err(anyhow!(
            "expected 2 positional args (<config.json> <fs_root>), got {}\n\n{USAGE}",
            argv.len()
        ));
    }
    Ok(Args {
...
mod tests {
    ...
    fn parse_args_two_positionals() {
        let p = parse_args(v(&["config.json", "/tmp/fs"])).unwrap();
    ...
    fn parse_args_rejects_wrong_arity() {
        let err = parse_args(v(&["just-one"])).unwrap_err();

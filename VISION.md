...
**Versus agent frameworks (LangChain, LlamaIndex, CrewAI, AutoGen, and the durable-graph generation including LangGraph + Deep Agents and Burr).** Libraries for assembling one or several agents inside an application. Even the durable-graph variants treat the graph as workflow topology over an episodic run — a job that starts, executes, and exits — not as a continuously-running program whose nodes idle and wake on signal. The pluggable virtual filesystems that ship with the newest of these are per-task scratch, not per-agent lifetime memory. The engine is an OS for running millions of long-lived agents, indefinitely, with shared infrastructure for scheduling, state, observability, and cost. Different layer, different problem.

**Versus the "agent OS" neighborhood (OpenFang, Rivet agentOS, AgentScope, Letta, ElizaOS).** This is the closest-shaped competition — each takes a serious crack at one of our axes. OpenFang and Rivet wrap kernel-style primitives (scheduler, sandbox, virtual filesystem, audit chain) around an agent fleet. AgentScope makes MCP and agent-to-agent protocols first-class. Letta and ElizaOS run agents continuously rather than episodically. None compose all of these into a single substrate, and none are organized around a graph of long-lived subagents reconciling outputs across time, with per-agent durable filesystems, with provenance as a runtime invariant, and with MCP traffic treated as a kernel scheduling concern at sibling-agent scale. The gap is in the *combination*, and the combination is the architecture.

**Versus vendor agent platforms (OpenAI Assistants, Anthropic Agent SDK, vendor "agent clouds").** Hosted, opaque, episodic, and tied to a single model vendor. The engine is open, transparent, continuous, and model-agnostic. We treat model vendors as substitutable suppliers, not as the substrate.

**Versus single-application vertical AI tools, including the credible research-agent stacks (Future House's Robin / PaperQA2 / Aviary in science, FinRobot in markets, vertical copilots in law and medicine).** The strongest existing work on *agent-produced research outputs* — citation-grounded, multi-agent, end-to-end — lives in these domain-vertical stacks. They set the bar for what an agent deliverable looks like in a single field. They are also not continuous, not graph-as-program, and not domain-agnostic substrate; they internally reinvent thin slices of the kernel we are building. We are the substrate they should have been built on. As the field matures, the best of them compose naturally as node templates and curated MCP servers on top of a shared kernel.

**Versus large multi-agent simulation frameworks (CAMEL, AgentSociety, Project Sid).** The only OSS work demonstrating thousands-to-millions of long-lived agents running continuously today lives in *simulation* — agent-society research, urban modeling, Minecraft sandboxes. These prove the runtime cost curves at the scale we target are reachable. They do not produce outputs against the real world, they do not source claims to evidence, and they do not expose a graph a human architect operates on. We treat them as proof points for the performance frontier, not as substrate competition.

**Versus quant systems and HFT infrastructure.** Deterministic compute graphs over numerical data. The engine is a generative reasoning graph over heterogeneous evidence. The two are complements: numerical signal feeds the engine; the engine produces structured views the numerical systems can act on.
